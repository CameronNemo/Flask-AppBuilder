...
def is_safe_url(ref, tgt):
        ref_url = urllib.parse.urlparse(ref)
        tgt_url = urllib.parse.urlparse(tgt)
        return (
            tgt_url.scheme in ('http', 'https')
            and ref_url.netloc == tgt_url.netloc
        )
...
class AuthView(BaseView):
    ...
    def redirect_next(self):
        ...
        if next_url and self.is_safe_url(request.host_url, next_url):
